...
export default function ProductCard({ product, onCart, onFavorite, isFavorite = false, cartCount = 0, cartDisabled = false, cartDisabledLabel = 'Indisponible' }) {
  return (
    <article className="group overflow-hidden rounded-2xl border border-line bg-white/10 shadow-glow transition hover:-translate-y-1 hover:bg-white/15">
      ...
      <div className="space-y-4 p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <h3 className="line-clamp-2 font-bold">{product.titre}</h3>
          </div>
          <button
            className={`rounded-full border border-line p-2 transition hover:bg-rose-400/15 ${isFavorite ? 'bg-rose-400/20 text-rose-200' : 'text-rose-200'}`}
        ...
        <div className="flex items-center justify-between gap-3 text-sm text-slate-300">
          <span className="flex items-center gap-1"><FiMapPin /> {product.localisation || 'En ligne'}</span>
        ...
        <div className="flex items-center justify-between">
          <strong className="text-xl text-white">{Number(product.prix).toLocaleString('fr-FR')} Ar</strong>
          <button
            className={`btn relative ${cartDisabled ? 'cursor-not-allowed border border-line bg-white/10 text-slate-400' : 'btn-primary'}`}
